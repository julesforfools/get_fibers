...
#--------------------------------------------------------------------------
# Create a button in the scene tab to recolor the Straightened Fibers according to pennation angle
#--------------------------------------------------------------------------
# Primary operator
...
class SCENE_PT_recolor_by_angle(bpy.types.Panel):
    ...
    bl_idname = "SCENE_PT_add_recolor_by_angle"
...
#--------------------------------------------------------------------------
# Create a button in the scene tab to recolor the Straightened Fibers according to fiber length
#--------------------------------------------------------------------------
# Primary operator
class OBJECT_OT_recolor_by_length(bpy.types.Operator):
    """Recolor all fibers according to fiber length"""
    bl_idname = "curve.recolor_by_length"
    bl_label = "Recolor by Fiber Length"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # create the gradient material
        if not "Gradient_Length" in bpy.data.materials:                          # If material doesn't exist yet,
            bpy.data.materials.new("Gradient_Length")                            # create it
        mat = bpy.data.materials["Gradient_Length"]                              # Assign material to variable
        mat.use_nodes = True                                                    # Use material nodes
        mat.diffuse_color = (100,255,0,255)                                     # Assign Viewport material color
        tree = mat.node_tree                                                    # Assign node tree to variable
        nodes = tree.nodes                                                      # Assign nodes to variable
        #list(bpy.data.materials["Gradient_Length"].node_tree.nodes)
        ...
        op_lengths = []
        for ob in bpy.data.collections['Straightened'].all_objects:
            op_lengths.append(ob.data["fiber_length"])
        node_ramp.color_ramp.elements[0].position = min(op_lengths)/max(op_lengths)
        node_ramp.color_ramp.elements[1].position = max(op_lengths)/max(op_lengths)
        # Math module division and by 90
        node_math.operation = "DIVIDE"
        node_math.inputs[1].default_value = max(op_lengths)
        # Give Value an input # not supported
        # node_value.outputs[0].default_value = 0
        # driv = node_value.outputs[0].driver_add("default_value")
        # driv.driver.expression = "100"
        # Give Attribute an input
        node_attribute.attribute_type = 'OBJECT'
        node_attribute.attribute_name = 'fiber_length'
...
# Append to scene panel
class SCENE_PT_recolor_by_length(bpy.types.Panel):
    """Another Panel for assigning a color gradient to Fibers"""
    bl_label = "Recolor by Length"
    bl_idname = "SCENE_PT_add_recolor_by_length"
    bl_space_type = "PROPERTIES"
    bl_region_type = "WINDOW"
    bl_context = "scene"
    ...
    def draw(self, context):
        ...
        # Big render button
        layout.label(text=" Recolor by Length:")
        row = layout.row()
        row.scale_y = 3.0
        row.operator("curve.recolor_by_length")

# Registration
def register():
    bpy.utils.register_class(OBJECT_OT_recolor_by_length)
    bpy.utils.register_class(SCENE_PT_recolor_by_length)
def unregister():
    bpy.utils.unregister_class(OBJECT_OT_recolor_by_length)
    bpy.utils.unregister_class(SCENE_PT_recolor_by_length)
if __name__ == "__main__":
    register()
